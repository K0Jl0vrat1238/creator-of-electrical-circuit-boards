...
для слоёв BLACK (дорожки) и RED (контур).
"""
import argparse
import cv2
import numpy as np
...
def rasterize_vector_paths(paths, width, height, thickness_px):
...
def analyze_topology(gt_comps, vec_comps, min_overlap_ratio=0.05):
    """
    ...
    # Для каждого векторного компонента ищем пересечения с GT
    ...
    for v_id, v_mask in vec_comps.items():
        for g_id, g_mask in gt_comps.items():
            ...
            gt_area = g_mask.sum()
            if intersection > min_overlap_ratio * gt_area:
...
def save_topology_report(gt_mask, vec_mask, breaks, shorts, save_dir, layer_name):
    ...
    # Подсветка разрывов (жёлтый)
    for b in breaks:
        if "gt_id" in b:
            # Упрощённо: просто красим все GT пиксели проблемных компонентов в жёлтый
            # Для точного позиционирования нужен доступ к маскам, но для отчёта достаточно overlay
            pass 

    # Краткий отчёт на картинке
...
def main():
    ...
    parser.add_argument("--tool-diameter", type=float, default=1.0, help="Диаметр фрезы (мм)")
    ...
    thickness_px = max(1, int(round(args.tool_diameter * px_per_mm)))
    
    # Если задан минимальный зазор, применяем морфологическое закрытие к GT,
    # чтобы игнорировать микро-разрывы < min_gap_px
    ...
    config = PipelineConfig(
        ...
        tool_diameter_mm=args.tool_diameter, drill_diameter_mm=0.8,
    ...
    print("🎨 Извлечение эталонных маск...")
    raw_masks = _classify_color_masks(rgba)
    gt_masks = _segment_color_masks(rgba, raw_masks=raw_masks)

    layers = {
        "black": ("paths_paths", result.paths_paths, "Дорожки (BLACK)"),
        "red":   ("cut_paths",   result.cut_paths,   "Контур (RED)")
    }

    print("\n" + "="*60)
    print("🔍 ТОПЛОГИЧЕСКАЯ ПРОВЕРКА СВЯЗНОСТИ")
    print("="*60)
    print(f"📏 Масштаб: {px_per_mm:.2f} px/mm | 🛠 Толщина фрезы: {thickness_px}px")
    if args.min_gap_px > 0:
        print(f"⚠️  Допустимый зазор: {args.min_gap_px}px (микро-разрывы игнорируются)")
    else:
        print(" Режим: Касание 1 пикселя = короткое замыкание")
    print()

    total_breaks = 0
    total_shorts = 0

    for color, (res_attr, vec_data, label) in layers.items():
        print(f"📦 Анализ слоя: {label}")
        gt_mask = gt_masks[color]
        if gt_mask.sum() == 0:
            print("  ⏭️ [SKIP] Слой пуст на оригинале.")
            continue

        # Очистка GT от микро-шума (если задан зазор)
    ...
    if total_breaks == 0 and total_shorts == 0:
        print("✅ ТОПЛОГИЯ ИДЕАЛЬНА. Плата готова к фрезеровке.")
    else:
        print("️  Обнаружены топологические ошибки. Проверьте overlay-файлы в отчёте.")

    print(f"\n Визуальные отчёты: {args.output_dir}")
