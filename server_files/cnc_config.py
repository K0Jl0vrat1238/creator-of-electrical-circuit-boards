...
AXES_CONFIG = {
    ...
    'X': {
        'step': 19, 'dir': 5, 'en': 7, 'endstop': 22, 
        'steps_per_mm': X_STEPS_MM, 'final_offset_mm': 2.0  # Отъезд 2 мм
    },
    'Y': {
        'step': 11, 'dir': 13, 'en': 15, 'endstop': 32, 
...
# Шайтан-мотор
M_MOTOR = 31 # Активируется на LOW

# Свет
LED_PIN = 29 # Включается при LOW

# Тормоза при подаче LOW выключаются, при HIGH встают как вкопанные, есть задержка работы реле
BRAKE_PIN_Z = 3
BRAKE_PIN_Y = 26
BRAKE_PIN_X = 24
...
# Стоп кнопка
STOP_BUTTON_PIN = 33 # Подтянуто 3v3 (резистор на плате), кнопка нормально разомкнутая, при стопе замыкается на GND и просаживает подтянутое напряжение, софтовая подтяжка не нужна